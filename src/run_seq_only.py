...
def get_args():
    ...
    # for BCE Loss
    parser.add_argument("--pos_weight", type=str, default=None, help="positive weight for bce")
    ...
    parser.add_argument("--not_seq_prepend_bos",  action="store_true",
                        help="not seq prepend_bos")
    parser.add_argument("--not_seq_append_eos", action="store_true",
                        help="not seq append_eos")
    parser.add_argument("--not_matrix_prepend_bos",  action="store_true",
                        help="not matrix prepend_bos")
    parser.add_argument("--not_matrix_append_eos", action="store_true",
                        help="not matrix append_eos")
    ...
    parser.add_argument("--embedding_complete_seg_overlap",  action="store_true",
                        help="overlap segment of seq")
    parser.add_argument("--matrix_embedding_exists",  action="store_true",
                        help="whether the embedding exists")

    # train with dev and test results
    parser.add_argument("--batch_with_seq_ids", action="store_true",  help="the batch input with seq_ids")
...
def create_logger(args):
    ...
    if args.local_rank in [-1, 0]:
        ...
        if args.vector_dirpath is not None:
            dir_path_list = list(args.vector_dirpath.split("#"))
            for dir_path in dir_path_list:
                if not os.path.exists(dir_path):
                    os.makedirs(dir_path)
        if args.matrix_dirpath is not None:
            dir_path_list = list(args.matrix_dirpath.split("#"))
            for dir_path in dir_path_list:
                if not os.path.exists(dir_path):
                    os.makedirs(dir_path)
    else:
...
def get_model(args):
    ...
    if args.seq_fc_size and args.seq_fc_size != "null":
        if args.seq_fc_size[0] == "[":
            model_config.seq_fc_size = eval(args.seq_fc_size)
        else:
            model_config.seq_fc_size = [int(v) for v in args.seq_fc_size.split(",")]

    if args.matrix_fc_size and args.matrix_fc_size != "null":
        if args.matrix_fc_size[0] == "[":
            model_config.matrix_fc_size = eval(args.matrix_fc_size)
        else:
            model_config.matrix_fc_size = [int(v) for v in args.matrix_fc_size.split(",")]
        if "matrix" in args.input_type:
            model_config.embedding_fc_size = model_config.matrix_fc_size

    if args.vector_fc_size and args.vector_fc_size != "null":
        if args.vector_fc_size[0] == "[":
            model_config.vector_fc_size = eval(args.vector_fc_size)
        else:
            model_config.vector_fc_size = [int(v) for v in args.vector_fc_size.split(",")]
        if "vector" in args.input_type:
            model_config.embedding_fc_size = model_config.vector_fc_size
    ...
    # for matrix
    args.matrix_prepend_bos = True
    if args.not_matrix_prepend_bos:
        args.matrix_prepend_bos = False
    args.matrix_append_eos = True
    if args.not_matrix_append_eos:
        args.matrix_append_eos = False

    # for pair
    model_config.self_atten = args.self_atten
    model_config.cross_atten = args.cross_atten

    model_config.max_position_embeddings = args.seq_max_length + int(args.seq_prepend_bos) + int(args.seq_append_eos)

    if args.classifier_size:
        model_config.classifier_size = args.classifier_size
    if args.pos_weight:
        if "," in args.pos_weight:
            args.pos_weight = [float(v) for v in args.pos_weight.split(",")]
            assert len(args.pos_weight) == model_config.num_labels
        else:
...
def main():
    ...
    batch_data_func = BatchConverter(
        ...
        matrix_prepend_bos=not args.not_matrix_prepend_bos,
        matrix_append_eos=not args.not_matrix_append_eos,
        matrix_add_special_token=args.matrix_add_special_token,
        batch_with_seq_ids=args.batch_with_seq_ids
    )
